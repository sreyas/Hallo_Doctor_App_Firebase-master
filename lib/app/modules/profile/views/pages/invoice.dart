...
// Determine the financial year based on the current date
  ...
  Future<void> _exportData(DateTime start,DateTime end) async {
    ...
    await FirebaseFirestore.instance.collection('Invoice').where('createdAt', isGreaterThanOrEqualTo:Timestamp.fromDate(start)).where('createdAt', isLessThanOrEqualTo: end).where('advisorId',isEqualTo:  DoctorService.doctor?.doctorId).get();
    final List<List<dynamic>> csvData = [];
    csvData.add([
      'Invoiceno','Invoice Date', 'Description','From Name', 'From Address','From State','From State Code','From GST No','To Name', 'To Address','To State','To State Code','To GST No','HSN/SAC',
